...
class FilterApiClient {
  ...
  add(String module, String id, String name,
    ...
    try {
      ...
      final url = SERVER_URL + baseUrl + '?module=$module&id=$id&name=$name';
      print("POST $url");
      var response =
  ...
  getDataValues(String module) async {
    try {
      print(SERVER_URL + baseUrl);
      final url = SERVER_URL + baseUrl + '/values?module=$module';
      print("GET $url");
      var response = await httpClient.get(
        url,
      );
      print(response.statusCode);
      if (response.statusCode == 200) {
        return (response.data as List).map((x) => FilterFieldNameValues.fromJson(x)).toList();
      } else
        throw Exception('Failed to load jobs from API');
    } catch (_) {
      throw Exception('Failed to load jobs from API');
